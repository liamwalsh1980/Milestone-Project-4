    ...
    if size:
        if item_id in list(cart.keys()):
            if size in cart[item_id]['items_by_size'].keys():
                ...
                messages.success(
                    request, f'Updated {product.name} quantityin size \
                        ({size.upper()}) to \
                ...
                messages.success(
                    request, f'Added {product.name} in size \
                        ({size.upper()}) to your shopping cart')
        else:
            cart[item_id] = {'items_by_size': {size: quantity}}
            messages.success(
                request, f'Added {product.name} in size \
...
def adjust_cart(request, item_id):
    ...
    if size:
        if quantity > 0:
            cart[item_id]['items_by_size'][size] = quantity
            messages.success(
                request, f'Updated {product.name} quantity in size \
                    ({size.upper()}) to \
                        {cart[item_id]["items_by_size"][size]}')
        else:
            ...
            if not cart[item_id]['items_by_size']:
                cart.pop(item_id)
            messages.success(
                request, f'Removed {product.name} in size \
                    ({size.upper()}) from your shopping cart')
    else:
        if quantity > 0:
            cart[item_id] = quantity
            messages.success(
